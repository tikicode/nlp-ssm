...
class CustomModelforSequenceClassification(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        if self.type == "full":
            ...
            # Take the mean of the last hidden state across the sequence dimension to get a single vector
            mean_last_hidden_state = torch.mean(last_hidden_state, dim=1)
            
            # Pass the mean vector through the classifier to get logits
            ...
            prefix_expanded = p.unsqueeze(0).expand(batch_size, -1, -1)
            ...
            inputs_embeds = torch.cat([prefix_expanded, input_embeddings], dim=1)  # concatenate the input embeddings and the prefix
            ...
            prefix_attention_mask = torch.ones(batch_size, p.size(0)).to('cuda')
...
def train(mymodel, num_epochs, train_dataloader, validation_dataloader, test_dataloder, device, lr, model_name):
    ...
    if mymodel.type == "head":
        ...
        classifier_params = mymodel.classifier.parameters()  # Get the parameters of the classifier (head)
        ...
        prefix_params = [mymodel.prefix]   # Assuming `mymodel.prefix` is a nn.Parameter for the prefix
        classifier_params = list(mymodel.classifier.parameters())  # Assuming `mymodel.classifier` holds the classifier parameters
    
        # Combine prefix parameters and classifier parameters
        combined_params = prefix_params + classifier_params
        ...
        custom_optimizer = torch.optim.AdamW(combined_params, lr=lr)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--type", type=str, default="prefix", choices=["auto", "full", "head", "prefix"], help="type of tuning to perform on the model")
