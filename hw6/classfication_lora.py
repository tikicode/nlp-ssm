...
def train(mymodel, num_epochs, train_dataloader, validation_dataloader, test_dataloder, device, lr, small_subset=False):
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in tqdm(enumerate(train_dataloader)):
            ...
            xent_loss = loss(predictions, labels)
    ...
    if not small_subset: plt.plot(epoch_list, dev_acc_list, 'g', label='valid')
    plt.xlabel('Training Epochs')
    plt.ylabel('Accuracy')
    plt.title('Training and Validation Accuracy')
    plt.legend()
    save_path = "overfit.png" if small_subset else f"base_full_lr={lr}_num_epochs={num_epochs}.png"
...
if __name__ == "__main__":
    ...
    pretrained_model, train_dataloader, validation_dataloader, test_dataloader = pre_process(args.model,
                                                                                             ...
                                                                                             args.small_subset)
    # LoRA
    peft_config = LoraConfig(task_type=TaskType.SEQ_CLS, inference_mode=False, r=16, lora_alpha=16, lora_dropout=0.05)
