...
class NPLMFirstBlock(nn.Module):
    ...
    def __init__(self, vocab_size, embed_dim, local_window_size, hidden_dim, dropout_p):
        ...
        self.linear = nn.Linear(local_window_size * embed_dim, hidden_dim).to(device)
        self.layer_norm = nn.LayerNorm(hidden_dim).to(device)
        self.dropout = nn.Dropout(dropout_p).to(device)

    def forward(self, inputs):
        embeddings = self.embeddings(inputs) 
        concat_embeddings = embeddings.view(1, -1)
        transform = F.tanh(self.linear(concat_embeddings))
        layer_norm = self.layer_norm(transform)
        final_embeds = self.dropout(layer_norm)
...
class NPLMBlock(nn.Module):
    ...
    def __init__(self, hidden_dim, dropout_p):
        ...
        self.linear = nn.Linear(hidden_dim, hidden_dim).to(device)
        self.layer_norm = nn.LayerNorm(hidden_dim).to(device)
        self.dropout = nn.Dropout(dropout_p).to(device)

    def forward(self, inputs):
        transform = F.tanh(self.linear(inputs))
        residuals = transform + inputs
        layer_norm = self.layer_norm(residuals)
        final_inputs = self.dropout(layer_norm)
...
class NPLMFinalBlock(nn.Module):
    ...
    def __init__(self, vocab_size, hidden_dim):
        ...
        self.linear = nn.Linear(hidden_dim, vocab_size, bias=False).to(device)

    def forward(self, inputs):
        transform = self.linear(inputs)
        log_probs = F.log_softmax(transform, dim=1)
